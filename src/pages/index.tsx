...
const Home: NextPage = () => {
  ...
  const handleClick = (e) => {
    let query = {
      ...
      colour: e.currentTarget.getAttribute("data-colour"),
    };

    e.preventDefault();
    router.push({
      pathname: e.currentTarget.getAttribute("data-href"),
      query: query,
    });
  };
  ...
  const todoLists = data.myTodoList?.map((element, i) => (
    <div key={element.id}>
      <div
        onClick={handleClick}
        data-href={"/viewTodoList"}
        data-id={`${element.id}`}
        data-title={`${element.title}`}
        data-colour={`${element.colorCode}`}
      >
        ...
        <div>
          ...
          {todoLists?.length > 0 ? (
            ...
            <CenterDiv>
              ...
              <p>
                But don't worry: You can click on the + button below to create
