...
const TodoListMenu: FC = (submittedData) => {
    ...
    const [currentColour, setCurrentColour] = useState(submittedData.initialColour)
    ...
    if (listMutationResult.loading) console.log(`Submitting... ${listMutationResult.data}`);

    if (listMutationResult.error) console.log(`Submission error! ${listMutationResult.error.message}`);
    if (itemMutationResult.error) console.log(`Submission error! ${itemMutationResult.error.message}`);
    ...
    const saveListItems = (data) => {
        ...
        console.log(data);
        itemList.map((e, i) =>
            addTodoItem({variables: {title: e.title, isDone: !!e.isDone, todoListId: data.id}})
        )
    ...
    const saveTodoList = () => {
        ...
        if (id !== null && id !== "") {
            ...
            addTodoList({variables: {title: title, ownerId: ownerId, colour: colour}});
            console.log({variables: {title: title, ownerId: ownerId, colour: colour}})
    ...
    return <div>
        <ColourDotsSpace>
            {
                colours.map((e,i) => <ColourItem key={e} name={e} onClick={handleColourPicker} style={{background: e}}></ColourItem>)
            }

        </ColourDotsSpace>
        <FABstyleDelete onClick={deleteList}>
            Delete
        </FABstyleDelete>
